...
const messageSlice = createSlice({
  ...
  reducers: {
    ...
    deleteAllMessagesRequest(state) {
      state.loading = true;
      state.error = null;
      state.message = null;
    },
    deleteAllMessagesSuccess(state, action) {
      state.messages = []; // Clear messages after deletion
      state.error = null;
      state.loading = false;
      state.message = action.payload;
    },
    deleteAllMessagesFailed(state, action) {
      state.error = action.payload;
      state.loading = false;
      state.message = null;
    },
...
export const getAllMessages = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      "https://mern-stack-portfolio-backend-code-7m37.onrender.com/api/v1/message/getall",
...
export const deleteMessage = (id) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.delete(
      `https://mern-stack-portfolio-backend-code-7m37.onrender.com/api/v1/message/delete/${id}`,
      {
        withCredentials: true,
      }
    );
...
export const deleteAllMessages = () => async (dispatch) => {
  try {
    dispatch(messageSlice.actions.deleteMessageRequest());
    const response = await axios.delete("https://mern-stack-portfolio-backend-code-7m37.onrender.com/api/v1/message/deleteall", {
      withCredentials: true,
    });
