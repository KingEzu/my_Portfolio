...
const projectSlice = createSlice({
  ...
  reducers: {
    getMyProjectsRequest(state, action) {
      state.projects = [];
      state.error = null;
      state.loading = true;
    },
    ...
    getMyProjectsFailed(state, action) {
...
export const getMyProjects = () => async (dispatch) => {
  dispatch(projectSlice.actions.getMyProjectsRequest());
  try {
    ...
    dispatch(
      dispatch(projectSlice.actions.getMyProjectsSuccess(response.data.projects)) // Correct the data key here

    );
    dispatch(projectSlice.actions.clearAllErrors());
  } catch (error) {
    dispatch(
      projectSlice.actions.getMyProjectsFailed(error.response.data.message)
    );
  }
};
...
export const addNewProject = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      "https://mern-stack-portfolio-backend-code-7m37.onrender.com/api/v1/project/add",
      data,
      {
        withCredentials: true,
...
export const deleteProject = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `https://mern-stack-portfolio-backend-code-7m37.onrender.com/api/v1/project/delete/${id}`,
      {
        withCredentials: true,
      }
    );
...
export const updateProject = (id, newData) => async (dispatch) => {
  ...
  try {
    const response = await axios.put(
      `https://mern-stack-portfolio-backend-code-7m37.onrender.com/api/v1/project/update/${id}`,
      newData,
      {
        withCredentials: true,
        headers: { "Content-Type": "multipart/form-data" },
